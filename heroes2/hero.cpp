...
int heroi::getbonus(ability_s v) const {
	...
	for(auto e : artifacts) {
		...
		if(t.type == Ability) {
			if(t.value == v)
				r += i;
			else if(t.value == SpellPowerKnowledge && (v == SpellPower || v == Knowledge))
				r += i;
			else if(t.value == AttackDefence && (v == Attack || v == Defence))
				r += i;
			else if(t.value == AllAbilities && (v == Attack || v == Defence || v == SpellPower || v == Knowledge))
				r += i;
		}
	}
	return r;
...
unsigned heroi::getlearn(variantcol* result, unsigned count) const {
	...
	for(unsigned i = 0; i < count; i++) {
		...
		if(!v)
			break;
		*((variant*)(result + n)) = v;
		result[n].count = skills[v.value] + 1;
		exclude.add((skill_s)v.value);
		n++;
	}
	return n;
}
...
void heroi::levelup(bool interactive) {
	...
	if(interactive) {
		...
		if(raise_count >= 1) {
			str.addn("\nВы также можете выучить %-1 %2",
				skill_level_name[raise_skills[0].count], raise_skills[0].getname());
		}
		if(raise_count >= 2) {
			str.adds("или %-1 %2",
				skill_level_name[raise_skills[1].count], raise_skills[1].getname());
		}
		str.add(".");
	...
	abilities[raise]++;
	if(choose != -1)
		skills[raise_skills[choose].value]++;
	level++;
}
...
bool heroi::cast(spell_s id, const variant& target, bool run, bool test_cost) const {
	...
	if(spell.is(Enchantment)) {
		...
		if(id == Hypnotize && is(GoldenWatch))
			power += getpower(GoldenWatch);
